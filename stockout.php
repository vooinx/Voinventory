...
if(isset($_POST['tambah_barang_keluar'])){
    ...
    if($stock_in_data['count'] == 0){
        ...
        // Cek stok tersedia berdasarkan total stock in - total stock out
        $cek_stok = mysqli_query($conn, "
            SELECT 
                (SELECT COALESCE(SUM(qty), 0) FROM barang_masuk WHERE id_stok = '$id_stok') as total_masuk,
                (SELECT COALESCE(SUM(qty), 0) FROM barang_keluar WHERE id_stok = '$id_stok') as total_keluar
        ");
        $data_stok = mysqli_fetch_assoc($cek_stok);
        $stok_tersedia = $data_stok['total_masuk'] - $data_stok['total_keluar'];
        ...
        if($stok_tersedia >= $qty){
            ...
            if(mysqli_query($conn, $query_keluar)){
                // KEMBALIKAN: Update stok di tabel stok (mengurangi stok)
            ...
            echo "<script>alert('Stok tidak mencukupi! Stok tersedia: $stok_tersedia');</script>";
        }
    }
}
...
// QUERY YANG DISEDERHANAKAN: Menampilkan barang yang bisa di-stock out
$result_stok = mysqli_query($conn, "
    SELECT DISTINCT s.id_stok, s.nama_barang,
           (SELECT COALESCE(SUM(qty), 0) FROM barang_masuk WHERE id_stok = s.id_stok) as total_masuk,
           (SELECT COALESCE(SUM(qty), 0) FROM barang_keluar WHERE id_stok = s.id_stok) as total_keluar,
           ((SELECT COALESCE(SUM(qty), 0) FROM barang_masuk WHERE id_stok = s.id_stok) - 
            (SELECT COALESCE(SUM(qty), 0) FROM barang_keluar WHERE id_stok = s.id_stok)) as stok_tersedia
    FROM stok s 
    WHERE s.id_stok IN (SELECT DISTINCT id_stok FROM barang_masuk)
                            ...
                            <div class="form-group">
                                ...
                                <select name="id_stok" required <?= $stok_kosong ? 'disabled' : '' ?>>
                                    ...
                                    <option value="<?= $stok['id_stok'] ?>">
                                        <?= $stok['id_stok'] ?> - <?= $stok['nama_barang'] ?> (Stok: <?= $stok['stok_tersedia'] ?>)
                                    </option>
                                    <?php endwhile; ?>
                                </select>
                            </div>
