...
if(isset($_POST['tambah_barang_masuk'])){
    ...
    // Hitung sisa yang bisa di-stock in
    $sisa_bisa_stockin = $data_stok['stok'] - $data_stok['net_stockin'];
    
    if($qty > $sisa_bisa_stockin){
        echo "<script>alert('Jumlah melebihi kapasitas! Maksimal yang bisa di-stock in: $sisa_bisa_stockin');</script>";
    } else {
        ...
        if(mysqli_query($conn, $query_masuk)){
            // TIDAK MENAMBAH STOK DI TABEL STOK - hanya mencatat transaksi
...
// QUERY YANG DIPERBAIKI: Menampilkan barang yang masih bisa di-stock in
// dengan memperhitungkan net stock in (masuk - keluar)
$result_stok = mysqli_query($conn, "
    SELECT s.id_stok, 
           ...
           IFNULL(SUM(bk.qty), 0) as total_keluar,
           (IFNULL(SUM(bm.qty), 0) - IFNULL(SUM(bk.qty), 0)) as net_stockin,
           (s.stok - (IFNULL(SUM(bm.qty), 0) - IFNULL(SUM(bk.qty), 0))) as sisa_bisa_stockin
    FROM stok s 
    LEFT JOIN barang_masuk bm ON s.id_stok = bm.id_stok
    LEFT JOIN barang_keluar bk ON s.id_stok = bk.id_stok
    GROUP BY s.id_stok 
    HAVING sisa_bisa_stockin > 0
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="alert alert-warning">
                            ...
                            Semua barang sudah mencapai kapasitas maksimal stock in.
                            <br><small><strong>Info:</strong> Barang hanya bisa di-stock in sampai mencapai stok awal.</small>
                        </div>
                        <?php endif; ?>
                            ...
                            <div class="form-group">
                                ...
                                <select name="id_stok" required>
                                    ...
                                    <option value="<?= $stok['id_stok'] ?>">
                                        <?= $stok['id_stok'] ?> - <?= $stok['nama_barang'] ?> (Stok: <?= $stok['stok_awal'] ?>)
                                    </option>
                                    <?php endwhile; ?>
                                </select>
                            </div>
